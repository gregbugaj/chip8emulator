...
public :
    ...
    uint8_t *video;  // This is memory[0xF00];
    uint8_t VF;  // Carry flag
    uint16_t I;  // I is 16 bits wide
    uint16_t PC;  // Program Counter
    uint8_t SP;  // Stack Pointer
    uint16_t S[16]; // Stack, max size 16 The stack is an array of 16 16-bit values
    ...
    static Chip8State *init() {
        ...
        s->video = (uint8_t *) calloc(Chip8State::VIDEO_SIZE, sizeof(char)); //&s->memory[0xF00]; // 3840
        s->SP = 0; // 0xFA0 = 4000
